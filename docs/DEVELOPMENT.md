...
| 备份管理 | ✅ | 创建备份、列表查看、恢复、删除 |
| 精选收藏 | ✅ | 一键标记精选（❤️）、精选筛选 |
| 快速转发 | ✅ | 一键转发到频道或对话 |
| 回顾统计 | 🔄 | 核心层完成（随机回顾/活动概要），待命令对接 |
...
**近期任务（1-2周）**：
1. **回顾统计命令对接** - 将 ReviewManager 接入 bot 命令（/review, /recap）
2. **批量操作UI** - 批量标签管理的交互界面（Inline Keyboard）
3. **代码优化** - 清理未使用的导入和冗余代码
4. **自动备份调度** - 定时自动备份功能
5. **智能去重** - MD5检测和内容相似度算法
