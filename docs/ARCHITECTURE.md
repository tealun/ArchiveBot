...
ArchiveBot 是一个基于 Telegram Bot 的个人内容归档系统，采用分层架构设计，实现内容的智能分类、多级存储和高效检索。

**核心定位**：单用户个人实例工具，每个人部署自己的 Bot，数据完全私有。

### 1.1 设计原则

- **简单优先**：核心功能简单可用，高级功能逐步扩展
- **个人实例**：一个 Bot 实例只服务一个人，无需用户管理
- **数据自主**：所有数据存储在本地或个人授权的网盘
- **成本可控**：利用免费服务，避免高额成本
- **隐私优先**：数据完全归属用户，可随时导出
- **可扩展性**：模块化设计，易于添加新功能
...
│  │  Telegram Bot API                                  │     │
│  │  • 接收用户消息（文本/文件/转发）                    │     │
│  │  • 处理命令 (/search, /tags, /stats 等)           │     │
│  │  • 响应用户操作（按钮点击、确认对话）                │     │
│  │  • 所有者身份验证（仅允许配置的 owner_id）          │     │
│  └────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
