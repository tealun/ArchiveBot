...
- 🌏 **多語言最佳化**：英語/簡體中文/繁體中文（含區域術語）
- 🔗 **連結智慧**：自動提取網頁標題、描述等詮釋資料
...
- `owner_id`: 你的 Telegram User ID（從 @userinfobot 取得）
...
5. **AI 服務**：使用 AI 功能時，您的內容會傳送至第三方 AI 服務商（xAI/OpenAI/Anthropic/阿里雲）。請確保遵守這些服務商的使用條款和隱私政策
6. **API 使用**：使用者需自行申請並合法使用各項第三方服務的 API 金鑰，因 API 濫用產生的後果由使用者自行承擔

### 智慧財產權與隱私

7. **著作權保護**：請勿使用本專案歸檔受著作權保護的內容，或侵犯他人智慧財產權的資料
8. **隱私尊重**：請勿未經授權歸檔他人的私密資訊或對話內容
9. **開源協議**：本專案採用 MIT License，但不包含任何擔保或保證

### 無擔保聲明

10. **按原樣提供**：本軟體按「原樣」提供，不提供任何明示或暗示的擔保，包括但不限於適銷性、特定用途適用性和非侵權性
11. **風險自負**：使用本專案產生的任何直接或間接損失（包括但不限於資料遺失、服務中斷、業務損失等），開發者概不負責
12. **安全風險**：雖然專案採取了安全措施，但任何軟體都可能存在未知漏洞。使用者應自行評估安全風險

### 法律合規

13. **地區法律**：請確保在您所在地區使用本專案符合當地法律法規
14. **禁止違法**：嚴禁使用本專案從事任何違法違規活動，包括但不限於傳播違法資訊、侵犯隱私、網路攻擊等
