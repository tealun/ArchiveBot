            ...
            if env_value is not None and env_value != '':
                ...
                if key == 'bot.owner_id':
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid {env_var} value: {env_value}, using YAML config")
                elif key in ['ai.exclude_from_context.channel_ids', 'ai.exclude_from_context.tags']:
                    # 处理列表类型的环境变量（JSON格式）
                    try:
                        import json
                        return json.loads(env_value)
                    except (json.JSONDecodeError, ValueError):
                        logger.warning(f"Invalid {env_var} value (not valid JSON): {env_value}, using YAML config")
                else:
                    return env_value
